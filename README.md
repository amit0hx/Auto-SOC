...
Auto-SOC bridges the gap between **raw network anomaly detection** and **actionable incident response**. It integrates two AI paradigms:

1. 🧠 A **Random Forest Classifier** analyzes network traffic logs and detects **5 types of attacks** (DoS, Probe, R2L, U2R)
2. 🤖 Detected threats are forwarded to **Google Gemini (LLM)** which generates technical **mitigation playbooks** with MITRE ATT&CK references
3. 📊 A **Streamlit Dashboard** provides real-time visualization, batch analysis, and attack history tracking
4. 📡 A **real-world capture bridge** (`live_ids.py`) feeds **live Wireshark/tshark traffic** through the same pipeline and **identifies attack sources by IP** (with reverse-DNS enrichment)
...
| 🎯 **Attack Source ID** | Identifies attacker **source IP**, internal/external origin, and reverse-DNS per detected attack |
...
> retraining on a real labelled corpus (e.g. CIC-IDS2017). See [`REAL_DEPLOYMENT_PLAN.md`](REAL_DEPLOYMENT_PLAN.md).
